...
- Run the daemon: `cargo build --release --bin soma-daemon`, then `./target/release/soma-daemon --config <path.toml>`. It expects a TOML config (default is platform-dependent: typically `~/.config/soma/daemon.toml` on Linux, resolved via `default_config_path()` → `dirs::config_dir()`); a minimal one sets `lif_count`, `izh_count`, `channels`, `tick_rate_hz`, `log_level`, `spine_sub_port`, `spine_pub_port`, and `model_path` (`model_path` is unused by the stub backend). With the stub backend it runs a headless SNN tick loop and logs `🔌 Using stub backend`.
