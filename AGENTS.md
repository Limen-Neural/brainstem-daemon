...
- Build / test / lint: `cargo build`, `cargo test --all-features` (14 tests), `cargo clippy --all-targets --all-features -- -D warnings`, `cargo fmt --check`.
...
- Run the daemon: `cargo build --release --bin soma-daemon`, then `./target/release/soma-daemon --config <path.toml>`. It expects a TOML config (default `~/.config/soma/daemon.toml`); a minimal one sets `lif_count`, `izh_count`, `channels`, `tick_rate_hz`, `log_level`, `spine_sub_port`, `spine_pub_port`, and `model_path` (`model_path` is unused by the stub backend). With the stub backend it runs a headless SNN tick loop and logs `🔌 Using stub backend`.
