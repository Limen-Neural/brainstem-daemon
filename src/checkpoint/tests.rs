...
fn live_rejects_sidecar_with_output_weights() {
    // Fail closed: neuromod 0.4 has no readout matrix, so a sidecar carrying
    // `output_weights` must be rejected loudly rather than silently dropping
    // trained readout weights on a "successful" load.
    let dir = unique_dir();
    let json = r#"{
            "source": "spikenaut_julia",
            "encoder": "v3_state_telemetry",
